...
def run_hkmeans(x, args, fea_dim=128):
    ...
    embedding_labels_set = x[:,emb_lenth:]
    embedding_labels = embedding_labels_set[:, args.selected_hie_level]
    x = x[:, 0:emb_lenth]
...
def compute_features(eval_loader, model, args, is_mlp_k=True, is_embedding_q=False, is_embedding_k=False, embedding_dim=2048):
    ...
    features_targets = torch.zeros(len(eval_loader.dataset), dim + args.num_hie_level).cuda()
    eval_bar = tqdm(eval_loader)
    with torch.no_grad():
        for i, (images, index, target_list) in enumerate((eval_bar)):
            images = images.cuda(non_blocking=True)
            feat = model(im_q=images, im_k=images, is_mlp_k=is_mlp_k, is_embedding_q=is_embedding_q, is_embedding_k=is_embedding_k)
            features_targets[index, 0:dim] = feat

            target = torch.cat(target_list, dim=0).reshape(args.num_hie_level, -1).float().T
            target = target.cuda(non_blocking=True)
            features_targets[index, dim:] = target

            eval_bar.set_description('Train mlp inference:')

    dist.barrier()
    dist.all_reduce(features_targets, op=dist.ReduceOp.SUM)
    return features_targets


def retrieval(model, val_loader, eval_loader, K, args, embedding_dim=2048):
    chunks=500
    print("Computing train features...(for knn)")
    model.eval()

    train_features_targets = torch.zeros(len(eval_loader.dataset), embedding_dim + args.num_hie_level).cuda()
    eval_bar = tqdm(eval_loader)
    with torch.no_grad():
        for i, (images, index, target_list) in enumerate(eval_bar):
            images = images.cuda(non_blocking=True)
            feat = model(im_q=images, im_k=images, is_embedding_q=True)
            train_features_targets[index, 0:embedding_dim] = feat

            target = torch.cat(target_list, dim=0).reshape(args.num_hie_level, -1).float().T
            target = target.cuda(non_blocking=True)
            train_features_targets[index, embedding_dim:] = target

            eval_bar.set_description('Train embedding inference:')

    print('train_features_targets size:',train_features_targets.size())


    print('Computing test features...(for recall)')
    test_features_targets = torch.zeros(len(val_loader.dataset), embedding_dim + args.num_hie_level).cuda()
    test_bar = tqdm(val_loader)
    with torch.no_grad():
        for i, (images, index, target_list) in enumerate(test_bar):
            images = images.cuda(non_blocking=True)
            feat = model(im_q=images, im_k=images, is_embedding_q=True)
            test_features_targets[index, 0:embedding_dim] = feat

            target = torch.cat(target_list, dim=0).reshape(args.num_hie_level, -1).float().T
            target = target.cuda(non_blocking=True)
            test_features_targets[index, embedding_dim:] = target  

            test_bar.set_description('Test embedding inference:') 

    print('test_features_targets size:',test_features_targets.size())


    recall_list = []
    k_max = np.max(K)
    # recall calculation    
    test_feat_norm = nn.functional.normalize(test_features_targets[:,0:embedding_dim], dim=1)
    split = torch.tensor(np.linspace(0, len(test_feat_norm), chunks + 1, dtype=int), dtype=torch.long).to(test_features_targets.device)
    for idx in range(args.num_hie_level):
        print(f'calculate recall at {idx}-level')
        test_label = test_features_targets[:,embedding_dim + idx]
        recall = [[] for i in K]
        ids = [torch.tensor([]).to(test_features_targets.device) for i in K]
        correct = [torch.tensor([]).to(test_features_targets.device) for i in K]
        with torch.no_grad():
    ...
    split = torch.tensor(np.linspace(0, len(test_feat_norm), chunks + 1, dtype=int), dtype=torch.long).to(test_feat_norm.device)
    knn_list = []
    # knn calculation
    train_feat_norm = nn.functional.normalize(train_features_targets[:,0:embedding_dim], dim=1)
    for idx in range(args.num_hie_level):
        print(f'calculate knn at {idx}-level')
        train_label = train_features_targets[:,embedding_dim + idx]
        test_label = test_features_targets[:,embedding_dim + idx]
        knn = [[] for i in K]
        correct = [torch.tensor([0]).to(test_features_targets.device) for i in K]
