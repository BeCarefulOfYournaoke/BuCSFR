...
vit_names = vits.__all__
model_names = list(model_names) + vit_names
# print(model_names)

parser = argparse.ArgumentParser(description="PyTorch ImageNet Training")
parser.add_argument('--dataset', type=str, default='cifar100', choices=['imagenet', 'cifar10', 'cifar100', 'cifartoy_good', 'cifartoy_bad', 'iNaturalist2019', 
                                                            'aircraft','cars196', "flowers102", "nabirds", "cub200"],
...
def main():
    ...
    # 'category_num' is the number of coarse categories in the dataset, and the coarse lables are used during training.
    # 'num_cluster' is the number of clusters for k-means when initializing the dendrogram, which is usually recommend set to 3-4 times the number of fine-grained classes.
    # 'threshold' is hyperparameter T in the paper, which is used to control the merging of clusters during constructing the dendrogram.
    # 'threshold' is dependent on the characristics of a dataset. 
    # For example, for fine-grained datasets, a small T is recommended, while for coarser datasets, a larger T is recommended. 
    if args.dataset == 'cifartoy_good':
        args.category_num = 2
        args.cluster_num_perc = 4*5
        args.num_cluster = args.category_num * args.cluster_num_perc
        args.threshold = 5
        args.good_split = True
        args.name_list = [
        'toy_1',
        'toy_2',
        ]
        ...
        args.category_num = 20
        args.cluster_num_perc = 5*5
        args.num_cluster = args.category_num * args.cluster_num_perc
        args.threshold = 2.0
        args.name_list = [
        'aquatic_mammals',
        ...
        args.category_num = 12
        args.cluster_num_perc = 500
        args.num_cluster = args.category_num * args.cluster_num_perc
        args.threshold = 5
        args.name_list = [
        'Invertebrate61',
        ...
        args.category_num = 30
        args.cluster_num_perc = 4*5
        args.num_cluster = args.category_num * args.cluster_num_perc
        args.threshold = 1.1
        args.name_list = ['ATR', 'Airbus', 'Antonov', 'Beechcraft', 'Boeing', 'Bombardier Aerospace', 
                          'British Aerospace', 'Canadair', 'Cessna', 'Cirrus Aircraft', 'Dassault Aviation', 'Dornier', 
                          'Douglas Aircraft Company', 'Embraer', 'Eurofighter', 'Fairchild', 'Fokker', 'Gulfstream Aerospace', 
                          'Ilyushin', 'Lockheed Corporation', 'Lockheed Martin', 'McDonnell Douglas', 'Panavia', 'Piper', 
                          'Robin', 'Saab', 'Supermarine', 'Tupolev', 'Yakovlev', 'de Havilland']

         
    elif args.dataset == 'cars196':
        args.category_num = 8
        args.cluster_num_perc = 25*5
        args.num_cluster = args.category_num * args.cluster_num_perc
        args.threshold = 1.1
        args.name_list = [
        'Cab18',
        ...
        args.category_num = 22
        args.cluster_num_perc = 25*2
        args.num_cluster = args.category_num * args.cluster_num_perc
        args.threshold = 1.1
        args.name_list = ["class_{i}" for i in range(22)]
    

    elif args.dataset == "cub200":
        args.category_num = 24
        args.cluster_num_perc = 8*5
        args.num_cluster = args.category_num * args.cluster_num_perc
        args.threshold = 1.1
        args.name_list = [
            "Waterbird",
    ...
    # wandb
    args.results_dir = f'arch_[{args.arch}]_data[{args.dataset}]_split_[{split}]_epochs[{args.epochs}]_memorysize[{args.moco_k}]_temperature_[{args.moco_t}]'
    args.wandb_id = args.dataset
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.eval:
        ...
        # retrieval_topk, knn_topk, prototype_knn_topk = utils.retrieval(model, val_loader, eval_loader, [1, 2, 5, 10, 50, 100, 200], cluster_result, args, embedding_dim)
        retrieval_topk, knn_topk, prototype_knn_topk = utils.retrieval(model, val_loader, eval_loader, [1, 2, 5, 10, 50, 100, 200], None, args, embedding_dim)
        ...
        if prototype_knn_topk is not None:
            prototype_knn_top1, prototype_knn_top2, prototype_knn_top5, prototype_knn_top10, prototype_knn_top50, prototype_knn_top100, prototype_knn_top200 = prototype_knn_topk
        ...
        if prototype_knn_topk is not None:
            print(f'prototype_knn@1: {prototype_knn_top1:.4f}, prototype_knn@2: {prototype_knn_top2:.4f}, prototype_knn@5: {prototype_knn_top5:.4f}, prototype_knn@10: {prototype_knn_top10:.4f},  prototype_knn@50: {prototype_knn_top50:.4f},prototype_knn100: {prototype_knn_top100:.4f}, prototype_knn@200: {prototype_knn_top200:.4f}')
    ...
    ####wandb
    wandb.init(project='ours', entity=args.wandb_id, name='train_' + args.results_dir, group=f'train_{args.dataset}')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        #after warmup, we compute the clustering result
        if epoch >= args.warmup_epoch:
            # compute momentum features for center-cropped images
            features = utils.compute_features(eval_loader, model, args, is_mlp_k=True, is_embedding_q=False, is_embedding_k=False, embedding_dim=embedding_dim) 
            ...
            if dist.get_rank() == 0:
                ...
                cluster_result = utils.run_hkmeans(features_cpu, args, fea_dim) 
        ...
        #recall test
        if (epoch + 1) % 10 == 0:
            retrieval_topk, knn_topk = utils.retrieval(model, val_loader, eval_loader, [1, 2, 5, 10, 50, 100, 200], None, args, embedding_dim)
            retrieval_top1, retrieval_top2, retrieval_top5, retrieval_top10, retrieval_top50, retrieval_top100, retrieval_top200 = retrieval_topk
            knn_top1, knn_top2, knn_top5, knn_top10, knn_top50, knn_top100, knn_top200 = knn_topk

            if retrieval_top1 > best_retrieval_top1:
                best_retrieval_top1 = retrieval_top1
            if retrieval_top2 > best_retrieval_top2:
                best_retrieval_top2 = retrieval_top2
            if retrieval_top5 > best_retrieval_top5:
                best_retrieval_top5 = retrieval_top5
            if retrieval_top10 > best_retrieval_top10:
                best_retrieval_top10 = retrieval_top10
            if retrieval_top50 > best_retrieval_top50:
                best_retrieval_top50 = retrieval_top50
            if retrieval_top100 > best_retrieval_top100:
                best_retrieval_top100 = retrieval_top100
            if retrieval_top200 > best_retrieval_top200:
                best_retrieval_top200 = retrieval_top200
            ...
            if knn_top1 > best_knn_top1:
                best_knn_top1 = knn_top1
            if knn_top2 > best_knn_top2:
                best_knn_top2 = knn_top2
            if knn_top5 > best_knn_top5:
                best_knn_top5 = knn_top5
            if knn_top10 > best_knn_top10:
                best_knn_top10 = knn_top10
            if knn_top50 > best_knn_top50:
                best_knn_top50 = knn_top50
            if knn_top100 > best_knn_top100:
                best_knn_top100 = knn_top100
            if knn_top200 > best_knn_top200:
                best_knn_top200 = knn_top200


            
            # save statistics
            print(f'Epoch [{epoch}/{args.epochs}]: R@1: {retrieval_top1:.4f}, R@2: {retrieval_top2:.4f}, R@5: {retrieval_top5:.4f}, R@10: {retrieval_top10:.4f},  R@50: {retrieval_top50:.4f},R@100: {retrieval_top100:.4f}, R@200: {retrieval_top200:.4f}')
            print(f'Epoch [{epoch}/{args.epochs}]: knn@1: {knn_top1:.4f}, knn@2: {knn_top2:.4f}, knn@5: {knn_top5:.4f}, knn@10: {knn_top10:.4f},  knn@50: {knn_top50:.4f},knn100: {knn_top100:.4f}, knn@200: {knn_top200:.4f}')
            train_logs.write(
            ...
            train_logs.write(
                f'Epoch [{epoch}/{args.epochs}]: K@1: {knn_top1:.4f}, K@2: {knn_top2:.4f}, K@5: {knn_top5:.4f}, K@10: {knn_top10:.4f},  K@50: {knn_top50:.4f},K@100: {knn_top100:.4f}, K@200: {knn_top200:.4f}\n')            
    ...
    print(f'[Best]: R@1: {best_retrieval_top1:.4f}, R@2: {best_retrieval_top2:.4f}, R@5: {best_retrieval_top5:.4f}, R@10: {best_retrieval_top10:.4f},  \
          R@50: {best_retrieval_top50:.4f},R@100: {best_retrieval_top100:.4f}, R@200: {best_retrieval_top200:.4f}')
    
    print(f'[Best]: knn@1: {best_knn_top1:.4f}, knn@2: {best_knn_top2:.4f}, knn@5: {best_knn_top5:.4f}, knn@10: {best_knn_top10:.4f},  \
          knn@50: {best_knn_top50:.4f},knn100: {best_knn_top100:.4f}, knn@200: {best_knn_top200:.4f}')
    
    train_logs.write(
        f'[Best]: R@1: {best_retrieval_top1:.4f}, R@2: {best_retrieval_top2:.4f}, R@5: {best_retrieval_top5:.4f}, R@10: {best_retrieval_top10:.4f},  \
          R@50: {best_retrieval_top50:.4f},R@100: {best_retrieval_top100:.4f}, R@200: {best_retrieval_top200:.4f}\n')
    train_logs.write(
        f'[Best]: K@1: {best_knn_top1:.4f}, K@2: {best_knn_top2:.4f}, K@5: {best_knn_top5:.4f}, K@10: {best_knn_top10:.4f},  \
          K@50: {best_knn_top50:.4f}, K: {best_knn_top100:.4f}, K@200: {best_knn_top200:.4f}\n')            
...
def train(train_loader, model, criterion, new_criterion, optimizer, epoch, args, cluster_result=None):
    ...
    for i, (images, index, labels, _) in enumerate(train_loader):
...
if __name__ == "__main__":
    ...
    print(f"consume : {elapsed_time:.6f} seconds")  # print elapsed time
